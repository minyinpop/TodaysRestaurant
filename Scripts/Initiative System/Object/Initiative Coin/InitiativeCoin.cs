...
namespace Initiative_System.Object.Initiative_Coin
{
    ...
    internal sealed class InitiativeCoin : CustomPointerEventHandler
    {
        ...
        private void MoveToThrowParent()
        {
            ...
            // Position
            var randomXPos = 1920 * Random.Range(.2f, .8f);
            var randomYPos = 1080 * Random.Range(.5f, .8f);
                
            var finalPos = new Vector2(randomXPos, randomYPos);

            // Angle Y
            var randomYTurns = Random.Range(12, 24);
            var finalYAngle = 180 * randomYTurns;
            
            // Angle Z
            var randomZTurns = Random.Range(12, 24);
            var randomZAngle = Random.Range(1, 360);
